...
import com.beeselmane.jgame.exception.PathFindException;
import com.beeselmane.jgame.mod.JavaModLoader;
import com.beeselmane.jgame.mod.ServerModList;
...
public class JavaGameServer {
    ...
    private static JavaGameServer server = null;
    ...
    public static JavaGameServer getServer() {
        return server;
    }

    public static void setupPath() throws PathFindException {
        String abstractPath = JavaGameServer.class.getProtectionDomain().getCodeSource().getLocation().getPath();

        if (abstractPath.endsWith("/")) {
            JavaGameServer.thisPath = abstractPath;
            File path = new File(JavaGameServer.thisPath);
            ...
            if (!path.isDirectory()) {
                ...
                JavaGameServer.thisJarFile = thisPath + jars.get(1);
            }
        } else if (abstractPath.endsWith(".jar")) {
            int finals = abstractPath.lastIndexOf("/");
            JavaGameServer.thisPath = abstractPath.substring(0, finals + 1);
            JavaGameServer.thisJarFile = abstractPath.substring(finals + 1);
        } else {
    ...
    public static void main(String[] args) {
        ...
        try {
            JavaGameServer.setupPath();
        } catch (PathFindException ex) {
            ex.printStackTrace(System.err);
        }

        JavaGameServer.server = new JavaGameServer();
    ...
    public synchronized void entry(String[] args) throws Exception {
        File modDir = new File(JavaGameServer.thisPath + "mods/");
        File modTmpDir = new File(JavaGameServer.thisPath + "mods/tmp/");
        File pluginDir = new File(JavaGameServer.thisPath + "plugins/");
        File configDir = new File(JavaGameServer.thisPath + "config/");
